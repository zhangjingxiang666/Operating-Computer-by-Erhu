...
while True: #死循环--一直运行
    ...
    avg_re = numpy.mean(Xdb[422])  # re音的基音【329.5Hz左右】在时变频谱矩阵中的行，为第422行左右。要是觉得不准，这个值可以自己调
    #print('re的强度为',avg_re)
    ...
    '''5. 操作鼠标'''
    if avg_re>-55:  #如果re音的基音（时变频谱矩阵中的第422行）振幅的平均值大于-55，则说明拉的是这个音。【觉得不准的话，这个阈值可以自己调】
        up_move()    #有re音的话则启动上面定义的up_move()函数，使鼠标向右移
    if avg_mi>-55:
        left_move()
    if avg_fa>-40:
        down_move()
